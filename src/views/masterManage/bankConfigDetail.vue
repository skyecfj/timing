  ...
  <div class="mainContent">
    ...
    <el-form :model="bankConfigForm" :label-position="labelPosition" label-width="100px" ref="bankConfigForm" :rules="rules">
      <el-form-item label="银行"  prop="bank" class="inline-item">
        <el-select size="small"  v-model.trim="bankConfigForm.bank" @change="handleChange" filterable placeholder="请选择" :disabled="disabled">
      ...
      <el-form-item label="银行编号"  prop="bankCode" class="inline-item">
        <el-input size="small" v-model.trim="bankConfigForm.bankCode" disabled></el-input>
      </el-form-item>
      <el-form-item label="文件上传" required :error="errorMsg">
        ...
        element-loading-background = "transparent">
          ...
          :on-success="uploadSuccess" :disabled="disabled">
      ...
      <el-form-item label="数据读取范围" required>
        <div class="range-set">
          <el-form-item prop="dataStartRow" class="item-line">
            <el-input size="small" v-model.trim="bankConfigForm.dataStartRow" :disabled="disabled">
           ...
           <el-form-item prop="dataEndRow" class="item-line">
            <el-input size="small"  v-model.trim="bankConfigForm.dataEndRow" :disabled="disabled">
              <template slot="prepend">第</template>
              <template slot="append">行</template>
            </el-input>
           </el-form-item>
      ...
      <el-form-item label="状态" prop="state">
        <el-radio-group v-model="bankConfigForm.state" :disabled="disabled">
        ...
        <template v-for="(item, index) in standardFormat">
          <el-form-item :label="item.name" :prop="item.modelParam" class="">
            <el-select size="small"
              ...
              filterable
              placeholder="请选择字段" :disabled="item.modelParam === 'transactionDate' || item.modelParam === 'bookDate' || disabled">
            ...
            <el-button size="small" type="success" class="c-button"
              ...
              v-if="item.modelParam === 'transactionDate' || item.modelParam === 'bookDate'" :disabled="disabled">清空</el-button>
          </el-form-item>
        </template>
      </el-form>
      <div class="modified">
        <div class="modified-item">
          <p class="name">更新时间</p>
          <el-input size="small" v-model="modifiedTime" disabled></el-input>
        </div>
        <div class="modified-item">
          <p class="name">更新人</p>
          <el-input size="small" v-model="modifiedUser" disabled></el-input>
        </div>
      </div>
    </div>
    <div class="opreRight" v-if="!disabled">
    ...
    <el-dialog title="设置" :visible.sync="dialogDateSet" class="setDateDialog">
      <el-form :model="amountForm" ref="amountForm" :label-position="labelPosition">
        <el-form-item label="时间类型" v-if="setDateType === 'bookDate'">
          <el-radio-group v-model="amountForm.content[0].dateType" :disabled="disabled">
          ...
          <div class="date-select">
        	  <div class="date-and-time" v-if="amountForm.content[0].dateType === 1">
        	  	<el-select size="small" v-model="amountForm.content[0].fileField" placeholder="请选择字段" :disabled="disabled">
                <el-option v-for="(item, index) in options" :value="item" :key="index">
                </el-option>
              </el-select>
        	    <el-select class="dataTime" size="small" v-model="amountForm.content[0].dateTime" :disabled="disabled">
              ...
              <el-select class="dataConPos" v-model="amountForm.content[0].dataCondition" placeholder="" :disabled="disabled">
                <el-option label="+" value="+"></el-option>
                <el-option label="-" value="-"></el-option>
                <el-option label="无" value=""></el-option>
              </el-select>
              <el-input v-if="amountForm.content[0].dataCondition === '+' || amountForm.content[0].dataCondition === '-'" class="datePos" placeholder="单行输入" size="small" v-model.trim="amountForm.content[0].value" :disabled="disabled"></el-input>
        	  </div>
              ...
              <el-select size="small" v-model="amountForm.content[0].date" :disabled="disabled">
        ...
        <el-form-item label="时间类型" v-else>
          <el-radio-group v-model="amountForm.content[1].dateType" :disabled="disabled">
            <el-radio :label="1">年月日&时分秒</el-radio>
          ...
          <div class="date-select">
        	  <div class="date-and-time" v-if="amountForm.content[1].dateType === 1">
        	  	<el-select size="small" v-model="amountForm.content[1].fileField" placeholder="请选择字段" :disabled="disabled">
                <el-option v-for="(item, index) in options" :value="item" :key="index">
                </el-option>
              </el-select>
        	    <el-select size="small" class="dataTime" v-model="amountForm.content[1].dateTime" :disabled="disabled">
                <el-option v-for="item in tradeDateTimeFormat" :key="item.id" :label="item.dictionaryEntryName" :value="item.dictionaryEntryName"></el-option>
             </el-select>
              <el-select size="small"  v-model.trim="amountForm.content[1].sequence" :disabled="disabled">
        	  ...
        	  <div class="date-and-time" v-else-if="amountForm.content[1].dateType === 3">
        	  	<el-select size="small" v-model="amountForm.content[1].fileFieldDate" placeholder="请选择字段" :disabled="disabled">
                <el-option v-for="(item, index) in options" :value="item" :key="index">
                </el-option>
              </el-select>
              <el-select size="small" v-model="amountForm.content[1].date" :disabled="disabled">
                <el-option v-for="item in tradeDateFormat" :key="item.id" :label="item.dictionaryEntryName" :value="item.dictionaryEntryName"></el-option>
              </el-select>
              <el-select size="small" v-model="amountForm.content[1].fileFieldTime" placeholder="请选择字段" :disabled="disabled">
                <el-option v-for="(item, index) in options" :value="item" :key="index">
                </el-option>
              </el-select>
              <el-select size="small" v-model="amountForm.content[1].time" :disabled="disabled">
            ...
            <div class="date-or-time" v-else>
              ...
              <el-select size="small" v-model="amountForm.content[1].dates" :disabled="disabled">
                <el-option v-for="item in tradeDateFormat" :key="item.id" :label="item.dictionaryEntryName" :value="item.dictionaryEntryName"></el-option>
              </el-select>
              <el-select size="small"  v-model.trim="amountForm.content[1].sequence" :disabled="disabled">
                <el-option v-for="item in orderOption" :key="item.id" :label="item.name" :value="item.name">
  ...
  methods:{
    ...
    clearSet (modelParam) {
      if (modelParam === 'bookDate') {
        ...
        this.fieldsForm.transactionDate = ''
      }
    },
    saveSet () {
  	...
  	getBankConfigInfo (id) {
  	  let that = this
      let param = [{
        name: 'id',
        value: id
      }]
      api.getBankConfigInfo(this.common.joinSearchParam(param),function(res){
        if (res.code === 200) {
          that.sameItem(that.bankConfigForm, res.data)
          that.sameItem(that.fieldsForm, res.data)
          that.options = res.data.headList
          that.oldBankId = res.data.bank
          that.setSavedFrom.content[0].dateType = res.data.bookDateType
          that.setSavedFrom.content[1].dateType = res.data.dateType
          that.setSavedFrom.content[0].fileField = res.data.bookDate
          that.modifiedTime = res.data.modifiedTime
          that.modifiedUser = res.data.modifiedUser;
          that.setSavedFrom.content[1].sequence =  res.data.sequence || '';
          that.amountForm.content[1].sequence =  res.data.sequence || '';
          if (res.data.bookDateType === 1) {
            that.setSavedFrom.content[0].dateTime = res.data.bookDateFormat
          } else {
            that.setSavedFrom.content[0].date = res.data.bookDateFormat
          }
          if (res.data.intervalDay > 0) {
          	that.setSavedFrom.content[0].dataCondition = '+'
          	that.setSavedFrom.content[0].value = res.data.intervalDay
          } else if (res.data.intervalDay < 0) {
            let intervalDay = res.data.intervalDay + ''
            that.setSavedFrom.content[0].dataCondition = '-'
          	that.setSavedFrom.content[0].value = intervalDay.replace('-', '')
          }
          if (res.data.dateType === 3) {
            let transactionDate = res.data.transactionDate
            let transactionDateFormat = res.data.transactionDateFormat
            that.setSavedFrom.content[1].fileFieldDate = transactionDate.split('&')[0]
            that.setSavedFrom.content[1].fileFieldTime = transactionDate.split('&')[1]
            that.setSavedFrom.content[1].date = transactionDateFormat.split('&')[0]
            that.setSavedFrom.content[1].time = transactionDateFormat.split('&')[1]

          } else {
            that.setSavedFrom.content[1].fileField = res.data.transactionDate
            that.setSavedFrom.content[1].dateTime = res.data.dateType === 1 ? res.data.transactionDateFormat : ''
            that.setSavedFrom.content[1].dates = res.data.dateType === 2 ? res.data.transactionDateFormat : ''
          }
          that.fileList = [{
    ...
    updateBankConfig () {
      ...
      this.$set(params, 'id', this.configId)
      ...
      if (this.setSavedFrom.content[0].dataCondition) {
        ...
        this.$set(params, 'intervalDay', '')
      }
      api.updateBankConfig(that.common.joinSearchParam(pathParam), params, function(res){
        if (res.code === 200) {
          that.showTips('更新成功','success')
          that.cancel ();
    ...
    cancel () {
      this.common.deleteTag(this.$route,this,'bankConfig')
    },
    save (formName) {
        ...
        if (valid) {
          if (this.errorMsg) {
            ...
            this.$refs['fieldsForm'].validate((valid) => {
              if (valid) {
                if (this.oldBankId == this.bankConfigForm.bank) {
                  this.updateBankConfig()
                } else {
                  this.checkBank()
                }
              } else {
