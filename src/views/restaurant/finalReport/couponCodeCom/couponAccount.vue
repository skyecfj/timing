<template>
  <div class="coupon-account">
    <div class="searchContent">
      ...
      <div class="selectContent">
        <div class="selectTitle">第三方</div>
          <el-select size="small"  v-model="thirdParty" filterable remote :remote-method="remoteMethod" v-scroll="scrollLoading" :loading="loading" @change="thirdChange">
          	<el-option value="" label="请选择"></el-option>
    ...
    <div class="dataContent" >
      ...
      <el-table :data="couponAccountDate" style="width: 100%" border>
        ...
        <el-table-column label="操作" width="200px">
          <template slot-scope="scope">
              ...
              <el-button size="small"   class="c-button-icon--primary--opacity c-button-icon-circle" @click="downloadList(scope.row.id)"
            ...
            <el-tooltip  content="详情" placement="top" effect="light" popper-class="table-cell-toolTip">
              <el-button size="small"   class="c-button-icon--primary--opacity c-button-icon-circle" @click="detailList(scope.row)"
...
export default {
  ...
  data () {
    return {
        ...
        disabledDate: (time) => {
          if (this.endDate) {
          	let curDate = (new Date(this.endDate)).getTime()
      ...
      pickerEnd: {
        disabledDate: (time) => {
          if (this.startDate) {
          	let curDate = (new Date(this.startDate)).getTime()
            let six = 180 * 24 * 3600 * 1000
            let sixMonths = curDate + six
  ...
  methods: {
    ...
    autoReport () {
      let that = this
      apidr.testCouponReport(function(res){
        if (res.code === 200) {
          that.getBalanceList()
    ...
    getBalanceList () {
      ...
      apidr.getBalanceList(this.common.joinSearchParam(pathParams), function(res){
      	that.couponAccountDate = []
      	that.totalRecords = 0
        if (res.code === 200) {
          that.couponAccountDate = res.data.list
    ...
    submits (formData) {
      ...
      let param = [{
        ...
        name: 'tradeStartTime',
        value: formData.tradeDate[0]
      }, {
        name: 'tradeEndTime',
        value: formData.tradeDate[1]
      }]
      apidr.handleCouponReport(this.common.joinSearchParam(param),function(res){
        if (res.code === 200) {
        	that.isShow = false
    ...
    deleteList (id) {
      ...
      this.$confirm('确定删除?', '提示', {
        ...
        that.delCouponList(id)
      }).catch(() => {
        that.common.infoMessage('已取消删除')
      })
    },
    detailList (row) {
      this.$router.push({name: 'couponCodeRep', query: {id: row.id, isHaveDetail: row.isHaveDetail}, params: {parentRoute:this.$route.fullPath}})
    },
    showTips (text,type) {
  ...
  filters: {
    checkBalanceCycle (val) {
      if (val.balanceStartDate === null) return ''
