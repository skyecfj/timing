<template>
  <div>
    ...
    <div class="search-content" >
      ...
      <div class="selectContent" v-if="tblType == 5 ">
        <div class="selectTitle">订单类型</div>
        <el-select size="small" v-model="orderType" placeholder="请选择订单类型">
      ...
      <div class="selectContent" v-if="tblType == 1 || tblType == 2 || tblType == 6">
        <div class="selectTitle">{{tblType == 8 ? '退款日期':'退券日期'}}</div>
        <el-date-picker v-model="refundTime" type="date"
              value-format="yyyy-MM-dd" placeholder="选择日期"></el-date-picker>
      </div>
      <div class="selectContent" v-if="tblType == 1 || tblType == 2 || tblType == 12 || tblType == 13 || tblType == 14 || tblType == 15">
      ...
      <div class="selectContent" v-if="tblType == 8 || tblType == 12 || tblType == 14">
      ...
      <div class="selectContent" v-if="tblType == 3 || tblType == 4">
    ...
    <div class="content">
      ...
      <template v-if="tabs.length == 0" >
        ...
        <checkDiffSingleTbl
                            ...
                            v-on:changePageInfo="getPageInfo"></checkDiffSingleTbl>
      </template>
      ...
      <template v-if="tabs.length > 0">
        <el-tabs v-model="activeTab" @tab-click="changeTab" >
          <el-tab-pane v-for="(tabItem,index) in tabs"
            ...
            <checkDiffSingleTbl v-bind:reportType="reportType"
                                v-bind:searchObj="searchObj"
                                v-on:selectedRow="selectedRow"
                                v-on:changePageInfo="getPageInfo"></checkDiffSingleTbl>
          </el-tab-pane>
...
<script>
  ...
  //单表
  import checkDiffSingleTbl from './detailTbl/checkDiffSingleTbl.vue'
	export default {
    name:'checkSumDiff',
    components:{
      checkDiffSingleTbl
    ...
    methods: {
      ...
      changeParams(){
        this.searchObj = {
          ...
          paySerialNumbers:this.transactionNo,
      ...
      exportDetail(){
        ...
        let storage = window.localStorage;
        let params  = JSON.parse(storage.params);
        let exportParams = Object.assign({},params,this.getDownloadCondition(params));
        delete exportParams.type;
        delete exportParams.businessType;
        var type = params.type
        var typeNumber = type.indexOf('-') < 0 ? type : type.split('-')[0]
        api.exportCheckDiff(typeNumber,exportParams,'application/vnd.ms-excel',function(res){
          if(res.code){
            ...
            link.href = res;
            if(_self.reportType == '4' )
            {
                link.download = '核对明细自有渠道虚拟卡平台金额不一致-发卡.xlsx'
            }else if (_self.reportType == '4-1'){
                link.download = '核对明细自有渠道虚拟卡平台金额不一致-退卡.xlsx'
            }else if (_self.reportType == '11'){
                link.download = '核对明细天猫商城虚拟卡平台金额不一致-发卡.xlsx'
            }else if (_self.reportType == '11-1'){
                link.download = '核对明细天猫商城虚拟卡平台金额不一致-退卡.xlsx'
            }else {
                link.download = '核对明细(金额不一致).xlsx';
            }
      ...
      getDownloadCondition(params){
        //默认params.type == 1 或者 2 或者6
        let detailParams = {
