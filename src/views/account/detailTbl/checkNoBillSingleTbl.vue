  ...
  export default{
    ...
    methods : {
      ...
      getData(){
        ...
        api.getNoBillDetail(typeNumber,searchParam,function(result){
      ...
      setTblHeaderConfig(){
        ...
        for(let i = 0; i < tblHeaderConfig.noBillTblHeader.length; i ++){
          if(tblHeaderConfig.noBillTblHeader[i].type == params.type){
            _self.tHeaderConfig = tblHeaderConfig.noBillTblHeader[i];
            if(tblHeaderConfig.noBillTblHeader[i].isSameHeader){
              _self.tHeaderConfig.labels = tblHeaderConfig.noBillTblHeader[i-1].labels;
              _self.tHeaderConfig.props  = tblHeaderConfig.noBillTblHeader[i-1].props;
            }
          }
        }
      },
