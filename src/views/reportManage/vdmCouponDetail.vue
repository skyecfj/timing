...
export default {
  ...
  methods: {
    ...
    searchList () {
      // this.couponDetailSearchData = JSON.parse(JSON.stringify(this.searchData))
      this.getVdmCouponDetail()
    },
    reset () {
    ...
    exportFile () {
      let that = this
      api.exportVdmCouponDetail(this.searchData, 'application/vnd.ms-excel', function(res){
        if (res.code) {
          ...
          link.download = '预售电子券码核销明细查询报表.xlsx'
    ...
    getVdmCouponDetail () {
      ...
      api.getVdmCouponDetail(this.common.joinSearchParam(pathParams),function(res){
    ...
    checkMail () {
      let that = this
      api.checkVdmCouponDetailMail(this.searchData, function(res){
        if (res.code === 200) {
    ...
    sendVdmCouponMail (val) {
      let that = this
      let params = Object.assign(this.searchData, { password: val })
      api.sendVdmCouponMail(params, function(res){
        if (res.code === 200) {
